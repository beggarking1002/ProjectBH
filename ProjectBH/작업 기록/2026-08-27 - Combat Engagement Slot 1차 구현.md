...
- 변경된 C++ 파일 전체 컴파일 통과
- UE 5.7 Editor Development 타깃 링크 성공
- PIE 동작 검증 필요

## 에디터 확인 절차

1. 테스트 구역의 NavMesh가 플레이어 기준 최소 300 cm 바깥까지 덮는지 `P` 키로 확인한다.
2. 같은 Enemy Blueprint를 12개 이상 배치한다.
3. PIE를 실행한다.
4. 4마리는 빨간 Attack Slot으로, 다음 8마리는 노란 Wait Slot으로 이동하는지 확인한다.
5. Attack Slot 적이 공격·회복 후에도 같은 슬롯을 유지하고 재공격하는지 확인한다.
6. Wait 적이 빈 Attack Slot으로 승격하는지 확인한다.

## 현재 범위와 후속 작업

슬롯 생성·예약·이동과 1차 교대·복구까지 구현했다.

- 공격자는 정상적인 공격과 회복이 끝난 뒤에도 같은 Attack Slot을 유지한다.
- 플레이어가 500 cm 이상 이동하면 Attack/Wait 역할은 유지하면서 각 Ring 내부 예약을 현재 위치 기준으로 재편성한다.
- 사망·경직·교착 시 명시적으로 슬롯을 반납한다.
- 같은 위치로 투영되는 슬롯 중복 방지와 다층 지형 판정은 평면 전투 테스트 이후 보강한다.
- 후속 구현에서 Holding Slot 16개와 중앙 승격 대기열을 추가했다. 세부 규칙은 [[2026-08-27 - Holding Ring 및 중앙 승격 대기열 구현]]을 따른다.

세부 검증 절차는 [[2026-08-27 - Detour Crowd 군중 검증 계획]]을 따른다.
